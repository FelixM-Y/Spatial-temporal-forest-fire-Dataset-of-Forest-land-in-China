...
算法原理：其采用基于热异常点（MODIS Active Fire）与地表反射率变化（ΔNBR 指数）的混合燃烧检测算法。热异常点探测基于 MODIS 传感器对地表热辐射的监测，能够快速识别可能存在火灾的区域，具有较高的时效性。地表反射率变化（ΔNBR 指数）则通过对比火灾前后地表植被在近红外与短波红外波段反射率的差异，利用植被在火烧前后的光谱特征变化来精确界定火烧迹地的范围。两者有机结合，充分发挥了热异常探测的及时性与光谱特征分析的
精确性优势，显著提高了火烧迹地检测的准确性与可靠性。​
